...
class ReCaptcha
{
    ...
    function __construct()
    {
        ...
        if (getenv('CAPTCHA_SITE_KEY') == NULL || getenv('CAPTCHA_SECRET_KEY') == "") {
            die("To use reCAPTCHA you must get an API key from <a href='"
                . $this->signup_url . "'>" . $this->signup_url . "</a>");
        }
        $this->_secret = getenv('CAPTCHA_SECRET_KEY');
        $this->_sitekey = getenv('CAPTCHA_SITE_KEY');
        if (getenv('CAPTCHA_LANG') == NULL || getenv('CAPTCHA_LANG') == "") {
            $this->_lang = 'en';
        } else {
            $this->_lang = getenv('CAPTCHA_LANG');
        }
    }
